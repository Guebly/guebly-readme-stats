...
/**
 * @file Contains a simple cloud function that can be used to check if the GitHub tokens are still
 * functional.
 *
 ...
 *
 * @param {boolean} up Whether the tokens are up or not.
...
/**
 * Cloud function that returns whether the GitHub tokens are still functional.
 *
...
export default async (req, res) => {
  ...
  try {
    ...
    switch (type) {
      case "shields":
        res.send(shieldsUptimeBadge(tokensValid));
        break;
      case "json":
        res.send({ up: tokensValid });
        break;
      default:
        res.send(tokensValid);
        break;
    }
  } catch (err) {
