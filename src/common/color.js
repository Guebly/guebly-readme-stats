 ...
 */
const isGradient = (colors) => {
  return (
    colors.length > 2 && colors.slice(1).every((color) => isHexColor(color))
  );
};
...
const fallbackColor = (color, fallbackColor) => {
  ...
  if (colors.length > 1 && isGradient(colors)) {
    gradient = colors;
  }

  return (
    (gradient ? gradient : isHexColor(color) && `#${color}`) || fallbackColor
  );
};
...
export { isHexColor, isGradient, resolveColors };
