...
import { ERROR_HINTS, RETRY_LATER_MSG } from "./error.js";
import { resolveColors } from "./color.js";
import { escapeHTML } from "./html.js";
import { clamp } from "./ops.js";
 ...
 */
const svgFlex = ({ items, gap, direction, sizes = [] }) => {
  let lastSize = 0;
 ...
 */
const languageBar = (langName, langColor) => {
  return `
 ...
 *
 * @param {Object} params Object that contains the progressBar parameters.
...
const progressBar = ({
  ...
  const progressPercentage = clamp(progress, 2, 100);
 ...
 */
const labeledIcon = (icon, label, testid, iconSize) => {
  ...
  return svgFlex({ items: [iconSvg, text], gap: 20 }).join("");
};

// Script parameters.
const ERROR_CARD_WIDTH = 576.5;

const UPSTREAM_ERRORS = [RETRY_LATER_MSG, ERROR_HINTS.MAX_RETRY];
 ...
 * @param {Parameters<typeof resolveColors>[0]["theme"]=} args.renderOptions.theme Card theme.
...
const buildErrorCard = ({
  ...
  return `
    <svg width="${ERROR_CARD_WIDTH}"  height="120" viewBox="0 0 ${ERROR_CARD_WIDTH} 120" fill="${bgColor}" xmlns="http://www.w3.org/2000/svg">
    <style>
    ...
    <text x="25" y="45" class="text">Something went wrong!${
      UPSTREAM_ERRORS.includes(secondaryMessage) || !show_repo_link
        ? ""
        : " file an issue at https://github.com/Guebly/guebly-readme-stats"
    }</text>
 ...
 */
const textWidth = (str, fontSize = 10) => {
  // prettier-ignore
  const widths = [
