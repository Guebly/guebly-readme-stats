 ...
 */
const cacheTTL = ({ requested, def, min, max }) => {
  let cacheSeconds = clamp(isNaN(requested) ? def : requested, min, max);

  if (process.env.CACHE_DURATION) {
    const envCacheSeconds = parseInt(process.env.CACHE_DURATION, 10);
 ...
 */
const applyCache = (res, cacheSeconds) => {
 ...
 * @param {any} res The response object.
 */
const applyErrorCache = (res) => {
  const envCacheSeconds = process.env.CACHE_DURATION
...
export { cacheTTL, applyCache, applyErrorCache, DURATIONS, CACHE_TTL };
