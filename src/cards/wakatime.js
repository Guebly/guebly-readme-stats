...
import { clamp, normalize } from "../common/ops.js";
import { progressBar, svgFlex } from "../common/render.js";
 ...
 */
const buildWakatimeCard = (stats = {}, options = { hide: [] }) => {
  let { languages = [] } = stats;
  const {
    hide_title = false,
    hide_border = false,
  ...
  if (shouldHideLangs) {
    const languagesToHide = new Set(hide.map((lang) => normalize(lang)));
    languages = languages.filter(
      (lang) => !languagesToHide.has(normalize(lang.name)),
    );
  }
  ...
  const langsCount = clamp(langs_count, 1, langs_count);
...
export { buildWakatimeCard };
export default buildWakatimeCard;
