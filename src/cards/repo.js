...
import { clamp, resolveEmojis } from "../common/ops.js";
import {
  svgFlex,
  textWidth,
  labeledIcon,
 ...
 */
const buildRepoCard = (repo, options = {}) => {
  const {
    name,
    nameWithOwner,
    description,
  ...
  const descriptionMaxLines = description_lines_count
    ? clamp(description_lines_count, 1, DESCRIPTION_MAX_LINES)
    : DESCRIPTION_MAX_LINES;

  const desc = resolveEmojis(description || "No description provided");
  const multiLineDescription = wrapText(
    desc,
  ...
  const svgLanguage = primaryLanguage ? languageBar(langName, langColor) : "";

  const totalStars = formatNumber(starCount);
  const totalForks = formatNumber(forkCount);
  const svgStars = labeledIcon(icons.star, totalStars, "stargazers", ICON_SIZE);
  const svgForks = labeledIcon(icons.fork, totalForks, "forkcount", ICON_SIZE);

  const starAndForkCount = svgFlex({
    items: [svgLanguage, svgStars, svgForks],
    sizes: [
      textWidth(langName, 12),
...
export { buildRepoCard };
export default buildRepoCard;
