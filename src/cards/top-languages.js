...
import { chunk, clamp, normalize } from "../common/ops.js";
import { progressBar, svgFlex, textWidth } from "../common/render.js";
...
const trimTopLanguages = (topLangs, langs_count, hide) => {
  ...
  let langsToHide = {};
  let langsCount = clamp(langs_count, 1, MAXIMUM_LANGS_COUNT);
  ...
  if (hide) {
    hide.forEach((langName) => {
      // @ts-ignore
      langsToHide[normalize(langName)] = true;
    });
  }
  ...
  langs = langs
    ...
    .filter((lang) => {
      // @ts-ignore
      return !langsToHide[normalize(lang.name)];
    })
    .slice(0, langsCount);
...
const createLanguageTextNode = ({
  ...
  const chunked = chunk(langs, langs.length / 2);
  ...
  const minGap = 150;
  const maxGap = 20 + textWidth(`${longestLang.name} ${percent}%`, 11);
  return svgFlex({
    items: layouts,
 ...
 */
const buildTopLangsCard = (topLangs, options = {}) => {
  const {
    hide_title = false,
    hide_border = false,
    card_width,
